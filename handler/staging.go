...
func StagingWebDAVFiber() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		// Intercept PATCH — emersion/go-webdav doesn't support it.
		// Handle via SabreDAV-style X-Update-Range convention.
		if string(c.Method()) == http.MethodPatch {
			// Build a minimal net/http request so we can reuse handleStagingPatch.
			bodyBytes := c.Body()
			stripped := strings.TrimPrefix(c.Path(), prefix)
			if stripped == "" {
				stripped = "/"
			}
			req, _ := http.NewRequest(http.MethodPatch, stripped, io.NopCloser(bytesReaderFromSlice(bodyBytes)))
			req.URL.Path = stripped
			req.Header.Set("X-Update-Range", c.Get("X-Update-Range"))
			rw := newFiberResponseWriter(c)
			handleStagingPatch(rw, req, sc.HostSrcDir)
			return nil
		}
...
// bytesReaderFromSlice returns an io.Reader over a byte slice without importing bytes
// at the top of this file (avoids cluttering imports for this single use).
...
// fiberResponseWriter adapts *fiber.Ctx to http.ResponseWriter so we can reuse
// the net/http handleStagingPatch implementation.
...
func newFiberResponseWriter(c *fiber.Ctx) *fiberResponseWriter {
	return &fiberResponseWriter{c: c, status: http.StatusOK, headers: http.Header{}}
}
