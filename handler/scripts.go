...
func ScriptFile(c *fiber.Ctx) (err error) {
	//host := c.Get("Host")
	...
	if helloJS == nil {
		...
		data, err := utils.ReadFileFromConfigPath(app.GetConfig().ScriptFolder, app.GetConfig().LocalScriptFilename)
		if err != nil {
			log.Errorf("error reading script file (%s): %s", err.Error())
		} else {
			script = string(data)
