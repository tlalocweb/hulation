...
func TestProxyDispatchPrecedence(t *testing.T) {
	...
	t.Run("by_domain wins regardless of config order", func(t *testing.T) {
		...
		handled := proxyDispatch([]*proxyRoute{pathRoute, domainRoute}, nil, nil, nil, func(*http.Request) bool { return false }, rec, req)
	...
	t.Run("by_path defers to a hula service route", func(t *testing.T) {
		...
		req.Host = "other.test"
		rec := httptest.NewRecorder()
		handled := proxyDispatch([]*proxyRoute{pathRoute}, nil, nil, nil, func(*http.Request) bool { return true }, rec, req)
		if handled {
	...
	t.Run("by_path handles when no hula route matches", func(t *testing.T) {
		...
		req.Host = "other.test"
		rec := httptest.NewRecorder()
		handled := proxyDispatch([]*proxyRoute{pathRoute}, nil, nil, nil, func(*http.Request) bool { return false }, rec, req)
	...
	t.Run("no match falls through", func(t *testing.T) {
		...
		req.Host = "nope.test"
		rec := httptest.NewRecorder()
		if proxyDispatch([]*proxyRoute{domainRoute, pathRoute}, nil, nil, nil, func(*http.Request) bool { return false }, rec, req) {
...
func TestProxyDispatchLongestPrefix(t *testing.T) {
	...
	for _, order := range [][]*proxyRoute{{short, long}, {long, short}} {
		...
		rec := httptest.NewRecorder()
		if proxyDispatch(order, nil, nil, nil, func(*http.Request) bool { return false }, rec, req); rec.Body.String() != "long" {
	...
	rec := httptest.NewRecorder()
	if proxyDispatch([]*proxyRoute{long, short}, nil, nil, nil, func(*http.Request) bool { return false }, rec, req); rec.Body.String() != "short" {
...
func TestProxyDispatchReservesServicePaths(t *testing.T) {
	...
	for _, reserved := range []string{"/api/health", "/scripts/x.js", "/analytics", "/v/foo"} {
		...
		rec := httptest.NewRecorder()
		if proxyDispatch(catchAll, nil, nil, nil, never, rec, req) {
	...
	rec := httptest.NewRecorder()
	if !proxyDispatch(catchAll, nil, nil, nil, never, rec, req) || rec.Body.String() != "proxy" {
