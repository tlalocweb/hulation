...
func TestChatRosterCRUD(t *testing.T) {
	...
	if _, err := PutChatRoster(StoredChatRoster{ServerID: "site-a"}); err != nil {
		t.Fatalf("clear: %v", err)
	}
	got, _ = GetChatRoster("site-a")
	if len(got.Recipients) != 0 {
		t.Fatalf("want empty after clear, got %v", got.Recipients)
	}

	// Delete is idempotent.
	if err := DeleteChatRoster("site-a"); err != nil {
